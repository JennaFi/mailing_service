...
from .models import Mailing, MailingAttempts, Recipient


def get_index_page_cache_data(user: User):
    ...
    if user.groups.filter(name='Manager').exists():
        mailing_attempt = MailingAttempts.objects.all()
        mailing = Mailing.objects.all()
        recipient = Recipient.objects.all()

    else:
        mailing_attempt = MailingAttempts.objects.filter(mailing__owner=user.id)
